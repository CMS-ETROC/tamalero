...
log_file_path = 'daq_run_readMode.log'
file_handler = RotatingFileHandler(log_file_path, maxBytes=6*1024*1024, backupCount=10)
...
def qinj_sender(fifo, QINJ_COUNT, stop_event, send_interval=0.05):
    print("Qinj sender thread started.")
    while not stop_event.is_set():
        try:
            fifo.send_Qinj_only(count = QINJ_COUNT)
            time.sleep(send_interval)
        except Exception as e:
            print(red(f"Error in qinj_sender thread: {e}"))
            break
        ...
        # while (datetime.now(timezone.utc) - start_time).total_seconds() < TEST_DURATION:
            try:
                ...
                if current_time - last_report_time >= report_interval:
                    ...
                    logger.info(f"Running time: {elapsed_time:.1f} seconds")
                    logger.info(f"Total cosmic hits: {hit_counter}")
                ...
                logger.error(f"An error occured in the daq loop: {e}", exc_info=True)
