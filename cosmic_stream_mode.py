...
log_file_path = 'daq_run_streamMode.log'
...
def qinj_sender(fifo, QINJ_COUNT, stop_event, send_interval=0.001):
...
def main():
    ...
    try:
        ...
        # while (datetime.now(timezone.utc) - start_time).total_seconds() < TEST_DURATION:
            try:
                ...
                for raw_data in fifo.stream(dispatch = True):
                    if check_for_quit():
                        break
    ...
    for etroc, _, _ in etroc_configs:
