...
def main():
    ...
    try:
        ...
        if not vtemp_channels:
            print(f"Error: Can't find VTEMP channal in config file {CONFIG_FILE_PATH} 道。")
            return
    ...
    if LOG_FILE:
        try:
            ...
            print(f"Vtemp data will be saved into {LOG_FILE}")
        except IOError as e:
            print(f"Error: Can't open file {LOG_FILE}: {e}")
            output_file = None
