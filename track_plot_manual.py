...
DEFAULT_Z_SPACING = 20.0  # mm - spacing between detector chips
BCID_WINDOW = 2
...
def create_hybrid_figure(track, z_spacing):
    ...
    # Add labels for each detector board
    ...
    # Draw detector board with transparent surface and grid
    ...
    # Draw each hit as a solid square with hover info
    for hit in track:
        x, y, z = hit['pos']
        # toa_cal_ns = calibrate_toa_ns(hit['toa'], hit['cal'])
        
        # Create hover text with all TDC information
        hover_text = (
            ...
            f"ToT: {hit['tot']}<br>"
            f"Cal: {hit['cal']}<br>"
            # f"<b>ToA (calibrated): {toa_cal_ns:.3f} ns</b>"
        )
    ...
    try:
        ...
        z_min, z_max = -1.5 * z_spacing, 4.5 * z_spacing
        t_min, t_max = (z_min - centroid[2]) / direction[2], (z_max - centroid[2]) / direction[2]
        ...
        fig_data.append(go.Scatter3d(x=[p_min[0], p_max[0]], y=[p_min[1], p_max[1]], z=[p_min[2], p_max[2]],
                                     mode='lines', line=dict(color='#00FF00', width=6))) # Bright Green
    except Exception as e:
        print(f"Could not fit line for track: {e}")
    
    # Calculate proper aspect ratio based on actual Z spacing
    z_range = 3 * z_spacing  # Total Z range (0 to 3*spacing for 4 boards)
    xy_range = 16  # Pixel range 0-15
    
    # Calculate Z aspect ratio
...
def update_track_view(prev_clicks, next_clicks, filter_value, current_state):
    ...
    if trigger_id == 'filter-radio': index = 0
        ...
        if trigger_id == 'next-button': index += 1
        elif trigger_id == 'prev-button': index -= 1
    
    if not active_tracks: return go.Figure(), "No tracks match filter", {'index': 0, 'filter': filter_value}
...
def display_hover_data(hoverData, current_state):
    ...
    try:
        # Get the active track list based on filter
        ...
        current_track = active_tracks[index % len(active_tracks)]
